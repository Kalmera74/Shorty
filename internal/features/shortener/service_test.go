...
func TestShortenURL_New_StoreCreateFails(t *testing.T) {
	...
	service := NewShortService(mockStore, mockRedis)
	_, err := service.ShortenURL(req)

	assert.Error(t, err)
	assert.IsType(t, &ShortenError{}, err)
...
func TestGetById_ValidId(t *testing.T) {
	...
	service := NewShortService(mockStore, mockRedis)
	result, err := service.GetById(types.ShortId(1))
...
func TestGetById_StoreFails(t *testing.T) {
	...
	mockStore.On("GetById", mock.Anything).Return(ShortModel{}, &ShortNotFoundError{})

	service := NewShortService(mockStore, mockRedis)
	_, err := service.GetById(types.ShortId(999))

	assert.Error(t, err)
	assert.IsType(t, &ShortNotFoundError{}, err)
	mockStore.AssertExpectations(t)
	mockRedis.AssertNotCalled(t, "Set")
}
...
func TestGetByShortUrl_EmptyUrl(t *testing.T) {
	...
	mockStore.On("GetByShortUrl", mock.Anything).Return(ShortModel{}, &ShortNotFoundError{})
	mockRedis.On("Get", mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return("", &ShortNotFoundError{})

	_, err := service.GetByShortUrl("")
...
func TestGetByShortUrl_CacheHit(t *testing.T) {
	...
	service := NewShortService(mockStore, mockRedis)
	result, err := service.GetByShortUrl(expectedShort.ShortUrl)
...
func TestGetByShortUrl_CacheMiss_DBMiss(t *testing.T) {
	...
	service := NewShortService(mockStore, mockRedis)
	_, err := service.GetByShortUrl("nonexistent")
...
func TestGetByLongUrl_ValidUrl_Hit(t *testing.T) {
	...
	result, err := service.GetByLongUrl("https://example.com")
	assert.NoError(t, err)
	assert.Equal(t, expectedShort, result)
	mockStore.AssertExpectations(t)
}
...
func TestGetByLongUrl_ValidUrl_Miss(t *testing.T) {
	...
	_, err := service.GetByLongUrl("https://example.com")
...
func TestSearch_ByOriginalUrl_NotFound(t *testing.T) {
	...
	mockStore.On("GetByLongUrl", url).Return(ShortModel{}, &ShortNotFoundError{})
...
func TestGetAllByUser_Found(t *testing.T) {
	...
	result, err := service.GetAllByUser(userID)
	assert.NoError(t, err)
...
func TestGetAllByUser_StoreError(t *testing.T) {
	...
	_, err := service.GetAllByUser(userID)
	assert.Error(t, err)
